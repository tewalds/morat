...
string GTP::solve_str(int outcome) const {
	switch(outcome){
...
GTPResponse GTP::gtp_solve_ab_params(vecstr args){
	if(args.size() == 0)
...
GTPResponse GTP::gtp_solve_ab_clear(vecstr args){
	solverab.clear_mem();
	return true;
}
...
GTPResponse GTP::gtp_solve_pns(vecstr args){
	double time = 60;
...
GTPResponse GTP::gtp_solve_pns_params(vecstr args){
	if(args.size() == 0)
		return GTPResponse(true, string("\n") +
...
GTPResponse GTP::gtp_solve_pns_stats(vecstr args){
	string s = "";
...
GTPResponse GTP::gtp_solve_pns_clear(vecstr args){
	solverpns.clear_mem();
	return true;
}


GTPResponse GTP::gtp_solve_pns2(vecstr args){
	double time = 60;
...
GTPResponse GTP::gtp_solve_pns2_params(vecstr args){
	if(args.size() == 0)
		return GTPResponse(true, string("\n") +
			"Update the pns solver settings, eg: pns_params -m 100 -s 0 -d 1 -e 0.25 -a 2 -l 0\n"
...
GTPResponse GTP::gtp_solve_pns2_stats(vecstr args){
	string s = "";
...
GTPResponse GTP::gtp_solve_pns2_clear(vecstr args){
...
GTPResponse GTP::gtp_solve_pnstt(vecstr args){
	double time = 60;
...
GTPResponse GTP::gtp_solve_pnstt_params(vecstr args){
	if(args.size() == 0)
		return GTPResponse(true, string("\n") +
...
GTPResponse GTP::gtp_solve_pnstt_clear(vecstr args){
