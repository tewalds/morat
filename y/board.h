...
public:
	...
	bool on_board_fast(int x, int y)   const { return (  y +   x < size_); }
	bool on_board_fast(const Move & m) const { return (m.y + m.x < size_); }
	...
	int line_end(int y)   const { return (size_ - y); }
