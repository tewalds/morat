...
public:
	...
	GTP(FILE * i = stdin, FILE * o = stdout) : GTPCommon(i, o), hist(Board(Board::default_size)) {
		...
		newcallback("state",           std::bind(&GTP::gtp_state,         this, _1), "Output the board state in a simpler form than print, or set the board state");
		newcallback("hash",            std::bind(&GTP::gtp_hash,          this, _1), "Output the hash for the current position");
		...
		newcallback("boardsize",       std::bind(&GTP::gtp_clearboard,    this, _1), "Alias for clear_board, board is fixed size");
		newcallback("size",            std::bind(&GTP::gtp_clearboard,    this, _1), "Alias for board_size");
